...
public class RandomFlyer : MonoBehaviour
{
    ...
    public AudioSource BirdMouth;
    public List<AudioClip> BirdSoundClips;
    public float MakeSoundFrequency = 5f;
    float timer = 0;
    #endregion
    ...
    void FixedUpdate()
    {
        if (makeSound)
        {
            if (timer == 0 || timer >= MakeSoundFrequency)
            {
                timer = 0;
                if (Random.Range(0, 101) >= 50)
                    BirdMouth.PlayOneShot(BirdSoundClips[Random.Range(0, BirdSoundClips.Count)]);
            }
        ...
        if (changeTarget < 0f)
        {
            rotateTarget = ChangeDirection(bodyTransform.position);
        ...
        if (bodyTransform.position.y < yMinMax.x + 10f ||
            bodyTransform.position.y > yMinMax.y - 10f)
        {
            if (bodyTransform.position.y < yMinMax.x + 10f) rotateTarget.y = 1f; else rotateTarget.y = -1f;
        }
        ...
        Vector3 rotation = Quaternion.RotateTowards(bodyTransform.rotation, lookRotation, turnSpeed * Time.fixedDeltaTime).eulerAngles;
        bodyTransform.eulerAngles = rotation;
        ...
        bodyTransform.Rotate(0f, 0f, prevz - temp, Space.Self);
        // Move flyer
        direction = Quaternion.Euler(transform.eulerAngles) * Vector3.forward;
        ...
        if (bodyTransform.position.y < yMinMax.x || bodyTransform.position.y > yMinMax.y)
        {
